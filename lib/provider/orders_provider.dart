...
class OrdersProvider {
  ...
  Future<List<Order>> getByClientAndStatus(String idClient, String status) async {
    try{
      Uri uri = Uri.http(_url, '$_api/findByClientIdAndStatus/${idClient}/${status}');
      Map<String, String> headers = {
        'Content-type': 'application/json',
        'Authorization': sessionUser!.sessionToken!
      };
  ...
  Future<List<Order>> getByStatus(String status) async {
    try{
      Uri uri = Uri.https(_url, '$_api/findByStatus/${status}');
      Map<String, String> headers = {
        'Content-type': 'application/json',
        'Authorization': sessionUser!.sessionToken!
      };
  ...
  */
  ...
  Future<ResponseApi> updateToCanceled(Order order) async {
    try {
      Uri uri = Uri.http(_url, '$_api/updateToCanceled');
      String bodyParams = json.encode(order);
      Map<String, String> headers = {
        'Content-type': 'application/json',
        'Authorization': sessionUser!.sessionToken!
  ...
  Future<ResponseApi> updateToLatLng(Order order) async {
    try {
      Uri uri = Uri.http(_url, '$_api/updateToLatLng');
      String bodyParams = json.encode(order);
      Map<String, String> headers = {
        'Content-type': 'application/json',
        'Authorization': sessionUser!.sessionToken!
